  ...
  function likeAdding() {
    if (!auth.currentUser) {
      ...
      const currentLikes = JSON.parse(props.likes),
        newLikes = liked
          ? [...currentLikes].filter((i) => i !== auth.currentUser.uid)
  ...
  function saving() {
    if (!auth.currentUser) {
      ...
      const currentSaved = JSON.parse(props.saved),
        newSaved = saved
          ? [...currentSaved].filter((i) => i !== auth.currentUser.uid)
          : [...currentSaved, auth.currentUser.uid];
      db.collection("posts")
        .doc(props.id)
        .set({
          caption: props.caption,
  ...
  return (
    <div className="Post">
      ...
      <span style={{ marginBottom: "10px" }}>
        {JSON.parse(props.likes).length} likes
      </span>
